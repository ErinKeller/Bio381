...
map<-qmap(location=c(lon = -72.897343, lat = 44.351406),zoom=9, maptype="toner-lite",source="stamen") # this assigns a variable map # you can edit the variables to get different map types (satellite, watercolor, tonor-lite, etc.)
                 
map + geom_point(data = points, aes(x = Longitude, y = Latitude), color="red", size=3, alpha=0.5) # ggmap allows you to add different variables, so here we are adding the points variable. 
...
points(x=points, pch=20, col="red") # can make different factors different colors
...
library(raster)
bioclimLC<-stack("bioclim-landuse1.tif") # this has 20 different files
names(bioclimLC)<-c("landcover","bio1","bio2","bio3","bio4","bio5","bio6","bio7","bio8","bio9","bio10","bio11","bio12","bio13","bio14","bio15","bio16","bio17","bio18","bio19") #renaming variable names from what it comes with
bioclimLC1<-crop(x=bioclimLC, extent(-74,-71, 42.5, 45.5)) # cropping it to Vermont's extent (otherwise you would get a picture of the western hemisphere)
plot(bioclimLC1)


# Using fake presence/absence data 
set.seed(44) 
